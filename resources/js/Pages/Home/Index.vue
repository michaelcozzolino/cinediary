<template>
    <guest>
        <template #main>
            ...
            <verify-email :status="status" :must-verify-email="mustVerifyEmail" />

            <login @on-forgot-password-button-click="this.openModal('forgot-password')" ref="login"/>
            <forgot-password ref="forgot-password" :status="status" />
            <reset-password @on-password-reset="this.openModal('login')" ref="reset-password" :email="email" :token="token" :status="status" />

            <how-it-works @on-button-click="this.openModal($event.refName)"
...
export default {
    components: {
        ...
        VerifyEmail,
        Popular,
        Guest, Jarallax, AppFooter, HowItWorks, Statistics, SectionTitle},
    props: {
        user: Object,
        status: String,
